...
export default function OnboardingPage() {
  ...
  }, [isConnected, address, publicClient, chain, walletClient]); // added walletClient

  const checkSmartAccount = async () => {
    if (!address || !publicClient || !chain) return;

    // Need a connected wallet client to predict the address via MetaMask toolkit
    if (!walletClient) {
      setDeploymentStatus("Connect your wallet to predict smart account");
      return;
    }
    ...
    try {
      ...
      if (cachedSmartAccount) {
        ...
        const { smartAccountAddress, isDeployed } = await res.json();
        if (isDeployed) {
          setSmartAccountAddress(smartAccountAddress);
          setIsDeployed(true);
          setDeploymentStatus(
            `Your smart account at ${smartAccountAddress} is already deployed`
          );
          return;
        }
      }
  ...
  const Step = ({
    ...
    num: number;
    title: string;
    status: "completed" | "active" | "loading" | "inactive";
    children: React.ReactNode;
  }) => {
    const statusStyles: Record<
      "completed" | "active" | "loading" | "inactive",
    ...
    > = {
      ...
      inactive: {
        ...
        icon: <div className="w-3 h-3 bg-gray-600 rounded-full" />,
      },
    };
    ...
    return (
      ...
      >
        <div
          className={`absolute left-0 top-0 flex items-center justify-center w-8 h-8 bg-black/50 rounded-full border-2 ${statusStyles[status].border}`}
        >
      ...
      <div className="relative z-10">
        ...
        <div className="relative z-20 pt-32 px-4 sm:px-6 lg:px-8 pb-20">
          ...
          <div
            ...
            style={{
              transform: monachadVisible ? "translateX(0)" : "translateX(100%)",
            }}
          >
            <img
              src="/pepemon6.png"
            ...
            <div className="bg-black/40 backdrop-blur-xl border border-purple-500/20 rounded-3xl p-8 shadow-2xl">
              ...
              >
                {!isConnected ? (
                  <>
                    <p className="mb-4">
                      Connect your wallet to own and manage your Smart Account.
                    </p>
                    <ConnectButton />
                  </>
                ) : (
                  <>
                    <p className="mb-2">
                      Your Externally Owned Account (EOA) is connected.
                    </p>
                    <p className="font-mono text-green-400 break-all text-xs bg-black/30 px-3 py-2 rounded-md border border-green-500/20">
                      {address}
                    </p>
              ...
              >
                {isConnected && (
                  <>
                    <p className="mb-4">
                      This action will deploy a new ERC-4337 smart contract
                      wallet, owned by your EOA. This enables secure,
                    ...
                    {smartAccountAddress && (
                      <div className="mb-4">
                        <p className="text-xs text-white/50 mb-1">
                          Deterministic Smart Account Address:
                        </p>
                        <p
                          className={`font-mono break-all text-xs px-3 py-2 rounded-md border ${
                            isDeployed
                              ? "text-green-400 bg-black/30 border-green-500/20"
                              : "text-yellow-400 bg-black/30 border-yellow-500/20"
                          }`}
                        >
                          {smartAccountAddress}
                        </p>
              ...
              <div className="relative pl-12">
                ...
                <h3
                  className={`text-xl font-semibold mb-3 ${
                    isDeployed ? "text-white/90" : "text-gray-600"
                  }`}
                >
                  Start Delegating
                </h3>
                <div
                  className={`text-sm leading-relaxed ${
                    isDeployed ? "text-white/70" : "text-gray-600"
                  }`}
                >
                  <p className="mb-4">
                    Your Smart Account is ready. You can now browse matches and
                    delegate to your favorite Monachads.
                  </p>
                  {isDeployed && (
                    <Link href="/matches">
                      <Button
              ...
              <div className="grid md:grid-cols-2 gap-8">
                ...
                <div className="bg-black/40 backdrop-blur-xl border border-purple-500/20 rounded-3xl p-8 shadow-2xl hover:border-purple-500/40 transition-all duration-300 hover:scale-[1.02]">
                  <h3 className="text-2xl font-semibold text-purple-400 mb-4">
                    Passive Income via Copy Trading
                  </h3>
                  <ul className="space-y-3 text-white/70 text-sm">
                    <li className="flex items-start">
                      <span className="text-purple-400 mr-3 mt-1">✓</span>
                      <span>
                        Automatically mirror the trades of top-performing
                        Monachads.
                      </span>
                    </li>
                    <li className="flex items-start">
                      <span className="text-purple-400 mr-3 mt-1">✓</span>
                      <span>
                        Maintain full custody of your funds with non-custodial
                        delegation.
                      </span>
                    </li>
                    <li className="flex items-start">
                      <span className="text-purple-400 mr-3 mt-1">✓</span>
                      <span>
                        Set strict, on-chain enforced rules like spending limits
                        and time-locks.
                      </span>
                    </li>
                    <li className="flex items-start">
                      <span className="text-purple-400 mr-3 mt-1">✓</span>
                      <span>
                        Explore different strategies by delegating to multiple
                        traders.
                      </span>
                    </li>
                  </ul>
                ...
                <div className="bg-black/40 backdrop-blur-xl border border-blue-500/20 rounded-3xl p-8 shadow-2xl hover:border-blue-500/40 transition-all duration-300 hover:scale-[1.02]">
                  ...
                  <ul className="space-y-3 text-white/70 text-sm">
                    <li className="flex items-start">
                      ...
                      <span>
                        Use your TCLUB tokens to vote on key platform decisions.
                      </span>
                    </li>
                    <li className="flex items-start">
                      <span className="text-blue-400 mr-3 mt-1">✓</span>
                      <span>
                        Delegate your voting power to influential community
                        members without transferring tokens.
                      </span>
                    </li>
                    <li className="flex items-start">
                      <span className="text-blue-400 mr-3 mt-1">✓</span>
                      <span>
                        Participate in 'Bribe Wars' to earn rewards for your
                        vote.
                      </span>
                    </li>
                    <li className="flex items-start">
                      <span className="text-blue-400 mr-3 mt-1">✓</span>
                      <span>
                        Influence the roadmap, fee structures, and new features.
                      </span>
                    </li>
                  </ul>
                </div>
